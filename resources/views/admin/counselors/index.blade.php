        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg p-6">
                ...
                <div class="mb-6">
                    ...
                    <p class="text-sm text-gray-600">Akun ini akan digunakan oleh tenaga medis untuk login di aplikasi mobile.</p>
                </div>
                ...
                <form action="{{ route('counselors.store') }}" method="POST" class="space-y-4">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                        <div>
                            ...
                            <x-text-input id="username" name="username" type="text" class="mt-1 block w-full" :value="old('username')" required placeholder="Username Konselor" />
                            <x-input-error :messages="$errors->get('username')" class="mt-2" />
                        </div>

                        <div>
                            <x-input-label for="email" value="Email" />
                            <x-text-input id="email" name="email" type="email" class="mt-1 block w-full" :value="old('email')" required placeholder="email@rsdunda.com" />
                    ...
                    <table class="w-full text-left border-collapse border border-gray-100">
                        <thead>
                            <tr class="bg-gray-50 text-gray-700 uppercase text-xs">
                                ...
                                <th class="p-4 border-b text-center">Aksi</th> </tr>
                        </thead>
                        <tbody class="divide-y divide-gray-100">
                            ...
                            <tr class="hover:bg-gray-50 transition-colors">
                                ...
                                <td class="p-4 text-sm text-gray-500 text-right">{{ $counselor->created_at->format('d M Y') }}</td>
                                <td class="p-4 text-center">
                                    <form action="{{ route('counselors.destroy', $counselor->id) }}" method="POST" onsubmit="return confirm('Apakah Anda yakin ingin menghapus akun konselor ini?')">
                                        @csrf
                                        @method('DELETE')
                            ...
                            @empty
                            <tr>
                                <td colspan="5" class="p-8 text-center text-gray-400 italic">Belum ada data konselor terdaftar.</td>
                            </tr>
                            @endforelse
                        </tbody>
                    </table>
